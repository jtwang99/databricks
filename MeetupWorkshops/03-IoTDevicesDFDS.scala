...
// MAGIC 
// MAGIC This dataset is avaialbe from Public [S3 bucket](//databricks-public-datasets/data/iot) or
// MAGIC 
// MAGIC from my [Gitbub](https://github.com/dmatrix/examples/blob/master/spark/databricks/notebooks/py/data/iot_devices.json)

// COMMAND ----------

// MAGIC %run ./datasets_mount

// COMMAND ----------

// MAGIC %fs ls /mnt/datariders

// COMMAND ----------

// MAGIC %fs head /mnt/datariders/devices.json
...
// MAGIC 
// MAGIC A schema defines columns names and types of a DataFrame. We are going to define a schema-on-read by telling Spark its types. First let's import all the types defined in Spark. The 
// MAGIC mother of all inclusive type is `StructType`, which can contain all other types or fields. The Spark supported types include: `LongType`, `DoubleType`, `ArrayType`, `IntegerType`,`StringType`, and `TimestampType`, etc.
...
// MAGIC %md To reflect our scheam above, let's create a Scala `case class` to represent our IoT device data.
...
// MAGIC %md Let's read the data from our SS location. 
// MAGIC **Note**: How fast the read operation took.
// MAGIC 
// MAGIC Why? Because `read` operation is a transformation, not an action where no job is created.

// COMMAND ----------

//fetch the JSON device information uploaded on our S3, now mounted on /mnt/datariders
val jsonFile = "dbfs:/mnt/datariders/devices.json"
...
// MAGIC %md Now here we are performing another transformation: `cache()`. The cache won't be materialized until an action is performed.
...
// MAGIC %md Now we perform an action, hence a Spark job will be executed, in which data will read, cache materialized, and the actual count computed and returned back to the driver.
...
//display Dataset in a nice columnar table. This is also considered an `action`
display(ds)
...
// MAGIC 
// MAGIC Note that `filter()`, as in RDD APIs, returns an immutable Dataset. The original dataset `ds` remains unchanged.

// COMMAND ----------

// MAGIC %md ####Q1: Can you find out all devices with temperatures excedding 30 and humidity greater than 70?

// COMMAND ----------

// issue select, map, filter, foreach operations on the datasets, just as you would for DataFrames
// it returns back a Dataset. Notice how we use DeviceIoTData's access fields with "." notation
// the code is a lot easier to read and express
...
// MAGIC %md ####CHALENGE: Using the [Dataset API](https://spark.apache.org/docs/latest/api/scala/index.html#org.apache.spark.sql.Dataset) methods execute some filter predicates, and display the new Dataset.
// MAGIC For example, try the `where()`, `select()`, `sort()`, etc.
// MAGIC * What happens when you use `select()`? 
// MAGIC * What do you get back? 
// MAGIC * Why is it so fast?

// COMMAND ----------

// MAGIC %md #### Q2: Can you display the first 10 items from the Datasets in sorted device_id descending order Q1?
...
// MAGIC %md #### Q3: Can you use the filter() used in Q2 and select the most hot and humid countries' sensors?
// MAGIC 
// MAGIC Hint: Use orderBy() and select columns temp, humidity, cca3, and devices
...
// MAGIC %md #### Remember in the lecture we talked about how Spark SQL undergoes logical and physical plans, to finally generate an optimized RDDs based physical plan of execution.
...
// MAGIC %md Apply higher-level Dataset API methods such as `groupBy()` and `avg()`. In other words, take all temperatures readings > 25, along with their corrosponding devices' humidity, groupBy cca3 country code, and compute averages. 

// COMMAND ----------

// MAGIC %md #### CHALLENGE: Compute the average humidity and temperature of all the devices in each country whose temperature and humidity are greater than 25 and 75 resepctively
// MAGIC 
// MAGIC Hint: Use `groupBy()` and `avg()`, `sort()`
...
// MAGIC 
// MAGIC By saving our Dataset as a table and caching it, I can issue complex SQL queries against it and visualize the results, using notebook's myriad plotting options.
...
// MAGIC %md Can you List the tables and databases?
...
// MAGIC %md Let's cache the table for efficient 
...
// MAGIC %md #### Q1: Count all devices for a partiular country and display them using Spark SQL
...
// MAGIC %md #### Q2: Do we get the same results if we use SQL?
...
// MAGIC %md #### Q3: How do we issue the same query as Q1 and Q2 using Dataset API?
...
// MAGIC %md #### Q1: How to drop columns you don't need, and save it as a parquet file?
// MAGIC 
// MAGIC Hint: Use Dataset drop() method and 
...
val parquetIotDevicesDF = spark.read.parquet("/tmp/iotDevicesParquet")
display(parquetIotDevicesDF)

// COMMAND ----------

// MAGIC %md ###CHALLENGE: Can you convert your Parquet read DataFrames into a Dataset?
// MAGIC 
// MAGIC HINT: You can start with the above case class.
...
// MAGIC %md ####HOME WORK CHALLENGE: Can you convert any of the above SQL queries to its equivalent Dataset/DataFrames
...
// MAGIC %md ####CHALLENGE: Feel the pain. Try translationg any of the above queries using RDDs API

// COMMAND ----------

// MAGIC %md ### Have fun & keep hacking...
